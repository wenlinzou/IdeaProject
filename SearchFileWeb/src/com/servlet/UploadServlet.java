...
//System.out.println("文件大小为：" + pContentLength + ",当前已处理：" + pBytesRead);
            ...
            for (FileItem item : list) {
                ...
                if (item.isFormField()) {
                    ...
                    if(!"xls".equals(fileExtName) && !"xlsx".equals(fileExtName)){
                        request.setAttribute("message", "上传失败,文件格式不正确!");
            ...
            if(null==filenames || filenames.size()<1){
                request.setAttribute("message", "上传失败,未上传文件!");
                request.getRequestDispatcher("/message.jsp").forward(request, response);
                return;
            }
            if(null!=filenames && filenames.size()==1){
                request.setAttribute("message", "上传失败,只上传了一个文件!");
                request.getRequestDispatcher("/otherInfo/judgeExcel/message.jsp").forward(request, response);
                return;
            }
            ...
            //判断excel内容
            List<String> htmlStrs = readExcelService.arrange2Excel(temprealSavePath);
            if(null != htmlStrs){
                if(null!=filenames){
                    System.out.println(filenames);
                    String judgeInfoTitle = "<h2 style=\"text-align:center;color:#4F515B;\">"+filenames.get(0)+" 和 "+filenames.get(1)+" 内容对比</h2>";
                    request.setAttribute("judgeInfo", judgeInfoTitle);
                }
                for (int i = 0; i < htmlStrs.size(); i++) {
                    request.setAttribute("table"+(i), htmlStrs.get(i));
                }
            }

            //清理旧的excel
            File oldFolder = new File(temprealSavePath);
            String folername = oldFolder.getParent();
            readExcelService.cleanOldFolder(folername);
    ...
    private String makePath(String filename, String savePath) {
        ...
        // 构造新的保存目录
        String dirtemp = getCurrentTimeAllNo();
//		String dir = savePath + "\\" + dir1 + "\\" + dir2; // upload\2\3
        // upload\3\5
        String dir = savePath + System.getProperty("file.separator") + dirtemp;
        // File既可以代表文件也可以代表目录
    ...
    public String getCurrentTimeAllNo(){
        Date now = new Date();
//		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");// 可以方便地修改日期格式
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMddHH");// 可以方便地修改日期格式
